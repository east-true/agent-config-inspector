...
별도 configuration surface로 추가한다. Copilot coding agent, code review, VS Code
표면은 계속 `unsupported`다.
...
Phase 8b의 custom agents, Phase 8c의 MCP inventory, 추가 provider는 각각 별도 설계와
release gate를 통과해야 한다. instruction resolution 결과와 configuration inventory를
하나의 effective prompt로 합치지 않는다.
...
- public Go API 범위
- Phase 8b custom agents와 Phase 8c MCP inventory의 세부 contract 및 출시 순서
