...
- Google Gemini CLI, Moonshot AI Kimi Code CLI, xAI Grok Build adapter
...
초기 범위는 provider 수를 두 개로 제한한다. 새 provider는 기존 adapter의 정확도와
...
| Grok Build | Phase 5 | baseline | 미포함 |
...
provider matrix가 Gemini, Kimi, Grok 순으로 넓어진다.
...
자기 phase에서 모델링해야 할 surface다. **14.1 Codex와 14.2 Claude만 초기 구현 범위**이며,
14.3 이후 절은 확장 방향을 미리 기록한 후속 설계다. 구현 시 공식 문서를 다시 확인하고
...
### 14.3 Google Gemini CLI 후속 adapter — Phase 3
...
첫 번째 후속 adapter로 사용한다. Phase 3 전에는 Gemini 관련 source를 발견하더라도
해석하지 않고 unknown candidate 또는 unsupported provider로만 보고한다.

### 14.4 Moonshot AI Kimi Code CLI 후속 adapter — Phase 4

Kimi adapter는 Phase 4에서 baseline resolution으로 처음 추가한다. 첫 공개 MVP에는
포함하지 않는다.

후속 모델링 대상:

- global 및 project-level `AGENTS.md` 계열 source
- project root부터 working directory까지의 merge 범위
- 하위 디렉터리의 관련 `AGENTS.md` 발견
- Kimi-specific instruction location이 문서화된 경우의 source identity
- launch-time system prompt가 repository parity에 미치는 영향의 metadata 표시
- provider가 보고한 CLI version

주요 edge case:

- root와 nested `AGENTS.md`가 동시에 존재
- project instruction과 global instruction의 우선순위
- Kimi-specific source와 일반 `AGENTS.md`가 동시에 존재
- launch parameter가 file-based instruction보다 높은 우선순위를 갖는 경우
- 문서 버전과 installed CLI의 동작이 일치하지 않음

Phase 4 release에서는 repository-owned source의 discovery, scope, ordering, provider 간
source diff를 제공한다. global context의 content 비교, 모든 launch override, behavioral
probe는 full support 승격 전까지 capability matrix에서 미지원 또는 documented로 표시한다.

### 14.5 xAI Grok Build 후속 adapter — Phase 5

Grok adapter는 Phase 5에서 baseline resolution으로 처음 추가하며 첫 공개 MVP에는
포함하지 않는다. Grok Build는 공식 문서상
...
Phase 5 release에서는 repository-owned file discovery와 documented ordering까지만 확정한다.
plugin, hook, skill, MCP 실행이나 전체 Claude compatibility emulation은 범위 밖이다.
...
| Code | 기본 severity | 의미 |
|---|---:|---|
| ACI001 | info | provider별 발견 source 목록 |
...
| ACI021 | warning | 같은 target에서 provider별 scope 결과가 다름 |
...
- Claude Code와 Codex CLI adapter만 기본 활성화
...
Case: root-agents-md-discovery
Repository access: generated fixture only
Filesystem mode: read-only where supported
...
3. 최소 read-only credential mount
4. 위 방법이 불가능하면 probe unsupported
...
위 tree는 초기 릴리스 구조다. Gemini, Kimi, Grok, Copilot 디렉터리와 fixture는 해당
provider phase가 시작될 때 추가한다. 빈 adapter stub을 미리 넣어 지원되는 것처럼 보이게
하지 않는다.
...
- Gemini, Kimi, Grok, Copilot 요청은 unsupported로 일관되게 표시
...
- launch/global context limitation 문서
- Kimi capability 및 evidence matrix

완료 기준:

- repository-owned source의 discovery, scope, ordering 결과를 설명 가능
- full로 검증하지 않은 launch/global behavior를 추측하지 않음
...
별도 설계 승인 후 시작한다. instruction resolver가 안정화되기 전에 범위를 확장하지 않는다.
...
- Gemini, Kimi, Grok, Copilot 요청의 명시적 unsupported 결과
...
추가 순서는 Gemini CLI, Kimi Code CLI, Grok Build, GitHub Copilot이다. 각 단계는 앞 단계가
release gate를 통과한 뒤 시작하며, 일정 때문에 여러 provider를 한 release로 합치지 않는다.
...
11. behavior probe에서 credential과 instruction home을 안전하게 분리하는 provider별 방법
12. `agent-config-inspector` repository, package, distribution namespace의 최종 사용 가능 여부
...
- Moonshot AI Kimi Code CLI customization:
  <https://www.kimi.com/help/kimi-code/cli-customization>
...
- Go 단일 binary를 우선한다.
- 초기 구현과 첫 개발자 preview, 첫 공개 MVP는 Claude Code와 Codex CLI만 지원한다.
- 초기 두 provider는 `full` 깊이로 구현하고, 다른 provider 이름만 미리 등록하지 않는다.
- 후속 provider는 Gemini CLI, Kimi Code CLI, Grok Build, GitHub Copilot 순으로 한 단계에
  하나씩 추가하며 각각 독립된 evidence와 release gate를 통과해야 한다.
...
- repository, package, distribution namespace의 최종 사용 가능 여부
- 세부 dependency
- glob compatibility 구현 방식
- Kimi와 Grok의 `full` support 승격 기준
- Copilot surface별 정식 지원 순서
- behavior probe의 provider별 credential isolation 방법
- public Go API 범위
