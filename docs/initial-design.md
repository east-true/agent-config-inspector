...
> 문서 상태: Living design — Phase 4 source implemented, v0.4.0 release preparation
>
> 문서 버전: 0.4.0-draft
>
> 기준일: 2026-07-24
...
현재 source에는 Phase 4의 Kimi Code CLI baseline adapter가 포함되어 v0.4.0 release gate를
검증 중이다. Grok Build와 GitHub Copilot은 계속 `unsupported`다.
...
14.3 Gemini는 Phase 3에서 구현·출시되었고, 14.4 Kimi는 Phase 4 source에 구현되었다.**
14.5 이후 절은 확장 방향을 미리 기록한 후속 설계다. 구현 시 공식 문서를 다시 확인하고
...
Kimi adapter는 Phase 4 source에서 baseline resolution으로 추가되었다. 첫 공개 MVP에는
포함되지 않았으며 v0.4.0 release gate를 통과한 뒤 공개 release에 포함한다. 대상 제품은
...
phase가 시작될 때 추가한다. 빈 adapter stub을 미리 넣어 지원되는 것처럼 보이게 하지
않는다.
...
상태: source 구현 완료, v0.4.0 release 준비
...
- v0.3.0부터 Gemini CLI adapter를 지원하며 Phase 4 source에는 Kimi Code CLI baseline
  adapter가 추가되어 v0.4.0 release를 준비한다.
- 초기 두 provider는 `full` 깊이로 구현하고, 다른 provider 이름만 미리 등록하지 않는다.
- 단계적 추가에서 Gemini CLI는 v0.3.0 release gate를 통과했고 Kimi Code CLI는 Phase 4
  source에 추가되었다. 이후 Grok Build와 GitHub Copilot을 한 단계에 하나씩 추가하고
  각각 독립된 evidence와 release gate를 통과해야 한다.
