...
> 문서 상태: Living design — Phase 8b custom agents inventory implemented, v0.8.0 development
>
> 문서 버전: 0.8.0-draft
>
> 기준일: 2026-07-24
...
repository-owned custom agents inventory를 추가한다. description과 prompt 및
...
상태: Phase 8a 완료(v0.7.0), Phase 8b 구현 완료(v0.8.0 개발 중)
...
Phase 8c의 MCP inventory와 추가 provider는 각각 별도 설계와 release gate를 통과해야 한다.
instruction resolution, Skills inventory, custom agents inventory를 하나의 effective prompt로
합치지 않는다. v0.8.0 tag와 release는 여러 후속 commit을 충분히 검증한 뒤 별도로 결정한다.
...
- public Go API 범위
- Phase 8c MCP inventory의 세부 contract 및 출시 순서
