...
- Moonshot AI Kimi Code CLI and xAI Grok Build adapters
...
14.3 Gemini는 Phase 3에서 구현되었다.** 14.4 이후 절은 확장 방향을 미리 기록한 후속
설계다. 구현 시 공식 문서를 다시 확인하고 evidence record와 golden fixture를 함께
갱신한다.
...
- Claude Code, Codex CLI, Gemini CLI adapter를 기본 활성화
...
위 tree는 초기 릴리스 구조다. Gemini 디렉터리와 fixture는 Phase 3에서 추가되었다.
Kimi, Grok, Copilot 디렉터리와 fixture는 해당 provider phase가 시작될 때 추가한다. 빈
adapter stub을 미리 넣어 지원되는 것처럼 보이게 하지 않는다.
...
상태: source 구현 완료, v0.3.0 release 준비
...
- Phase 3 source와 v0.3.0부터 Gemini CLI adapter를 추가한다.
- 초기 두 provider는 `full` 깊이로 구현하고, 다른 provider 이름만 미리 등록하지 않는다.
- 단계적 추가는 Gemini CLI로 시작했으며, 이후 Kimi Code CLI, Grok Build, GitHub
  Copilot을 한 단계에 하나씩 추가하고 각각 독립된 evidence와 release gate를 통과해야 한다.
